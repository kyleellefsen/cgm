...
class VizState:
    ...
    @property
    def current_samples(self) -> Optional[sampling.SampleArray]:
    ...
    def get_node_samples(self, node_name: str) -> Optional[List[int]]:
        ...
        if self.current_samples is None:
            return None
        try:
            node_idx = self.current_samples.schema.var_to_idx[node_name]
            return self.current_samples.values[:, node_idx].tolist()
    ...
    def get_node_distribution(self,
        ...
        if self.current_samples is None:
            return None
        schema = self.current_samples.schema
        node_idx = schema.var_to_idx[node_name]
        samples = self.current_samples.values[:, node_idx]
    ...
    @property
    def current_graph(self) -> Optional[cgm.CG]: 
        """Get the current graph."""
        return self._current_graph

    @property
    def current_graph_state(self) -> Optional[cgm.GraphState]: 
        """Get the current graph state."""
        return self._current_graph_state

    @property
    def conditioned_nodes(self) -> Dict[str, int]:
        """Get the currently conditioned nodes and their values."""
    ...
    def condition(self, node: str, value: Optional[int] = None) -> bool:
        ...
        if node not in self._current_graph_state.schema.var_to_idx: 
            return False
