...
export class GraphVisualization {
    ...
    constructor() {
        ...
        this.simulation = d3.forceSimulation<SimulationNode>()
            ...
            .force("collide", d3.forceCollide<SimulationNode>().radius(50))
    ...
    updateData(newData: { nodes: SimulationNode[]; links: SimulationLink[] }) {
        ...
        newData.links.forEach(link => {
            const sourceNode = this.simulationNodes.get(link.source.id);
            const targetNode = this.simulationNodes.get(link.target.id);
        ...
        this.simulation
            .nodes(nodes)
            .force("link", d3.forceLink<SimulationNode, SimulationLink>(links).id((d: SimulationNode) => d.id).distance(150))
